...
const MobileNumpad: React.FC<MobileNumpadProps> = ({ onKeyPress, currentInput }) => {
  ...
  return (
    <div className="absolute bottom-2 left-2 right-2 z-50">
      <div className="bg-slate-900/95 backdrop-blur-md rounded-2xl p-3 border border-cyan-400/30 shadow-2xl shadow-cyan-400/20">
        {/* Current input display */}
        <div className="bg-slate-800/70 border border-slate-500/50 px-3 py-2 rounded-lg text-center mb-3">
        ...
        <div className="space-y-2">
          {keys.map((row, rowIndex) => (
            <div key={rowIndex} className={`flex gap-2 justify-center ${rowIndex === 2 ? 'justify-center' : ''}`}>
              {row.map((key) => (
                <button
                  key={key}
                  onClick={() => handleKeyPress(key)}
                  className={`${rowIndex === 2 ? 'w-16' : 'w-14'} h-14 rounded-xl font-bold text-xl transition-all duration-200 transform active:scale-95 border ${
                    key === '⌫'
